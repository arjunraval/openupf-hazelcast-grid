...
log_prefix = "[Cookbook - Hazelcast_Grid_Setup][Recipe - hazelcast_jar] : "
domain_dir = node['hazelcast']['domain.dir']
checksum_dir = node['javamsp']['checksum.file.location']
checksum_file_name = "Hazelcast_Grid_Setup.hazelcast_jar.checksum"
appUser = node['javamsp']['appusers'][0][0]
appGroup = node['javamsp']['appgroups'][0][0]

## Installing hazelcast-spring Jar
package_loc = node['hazelcast']['hazelcast.jar.loc']
package_version = node['hazelcast']['hazelcast.jar.version']
package_name = node['hazelcast']['hazelcast.jar.name']
