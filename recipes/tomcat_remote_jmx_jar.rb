...
log_prefix = "[Cookbook - Hazelcast_Grid_Setup][Recipe - tomcat_remote_jmx_jar] : "
domain_dir = node['hazelcast']['domain.dir']
checksum_dir = node['javamsp']['checksum.file.location']
checksum_file_name = "Hazelcast_Grid_Setup.tomcat_remote_jmx_jar.checksum"
appUser = node['javamsp']['appusers'][0][0]
appGroup = node['javamsp']['appgroups'][0][0]

## Installing tomcat_remote_jmx_jar Jar
package_loc = node['hazelcast']['tomcat.remote.jmx.jar.loc']
package_version = node['hazelcast']['tomcat.remote.jmx.jar.version']
package_name = node['hazelcast']['tomcat.remote.jmx.jar.name']
...
execute "#{log_prefix} Validating checksum and installing package" do
  ...
  if [ \"`cat #{checksum_dir}/#{checksum_file_name}`\" == \"`cat #{checksum_dir}/#{checksum_file_name}.current`\" ] && [ \"\" != \"`cat #{checksum_dir}/#{checksum_file_name}`\" ]; then  
    ...
    echo \"Download tomcat.remote.jmx\";
